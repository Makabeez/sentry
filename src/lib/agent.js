...
addEntrypoint({
    key: "health",
    description: "Read the current Aave V3 health factor for an address. Free — you should " +
    ...
    handler: async (ctx) => {
        ...
        return {
            output: {
                ...
                healthFactor: raw > UNLIMITED ? null : Number(raw) / 1e18,
...
addEntrypoint({
    key: "watch",
    description: "Register an Aave V3 position for liquidation defence. The agent polls the " +
    ...
    handler: async (ctx) => {
        ...
        if (input.targetAfter <= input.actAt) {
            throw new Error(`targetAfter (${input.targetAfter}) must exceed actAt (${input.actAt}). ` +
                `Without a gap the trigger re-arms the moment it fires and repays on ` +
                `every poll.`);
        }
        const wallet = await agent.keeperhub.wallet();
        const { result } = await agent.keeperhub.read({
            protocol: "aave-v3",
            action: "get-user-account-data",
            args: { user: input.user },
        });
        ...
        return {
            output: {
                ...
                note: "Repayment is funded from the executing address above, not from the " +
