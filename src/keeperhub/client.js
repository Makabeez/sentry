 ...
 */
import { BudgetExceededError, KeeperHubError, SUPPORTED_CHAINS, VerificationError, } from './types.js';
const DEFAULTS = {
...
export class KeeperHubClient {
    ...
    constructor(config, fetcher) {
        ...
        if (!this.fetcher) {
            throw new KeeperHubError('No fetch implementation available. Pass one to the constructor on Node < 18.', 'CONFIG_NO_FETCH');
        }
    }
    // ---------------------------------------------------------------- transport
    async call(path, body, attempt = 0) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new KeeperHubError(`KeeperHub ${path} returned ${response.status}${detail ? `: ${detail}` : ''}`, `HTTP_${response.status}`);
            }
            ...
            throw new KeeperHubError(`KeeperHub ${path} failed after ${attempt + 1} attempts`, 'TRANSPORT_FAILED', undefined, error);
        }
        finally {
            clearTimeout(timer);
        }
    ...
    async read(request) {
        ...
        const response = await this.call(`/api/execute/${request.protocol}/${request.action}`, { chainId, ...request.args });
    ...
    async wallet() {
        ...
        if (!this.config.executingAddress) {
            throw new KeeperHubError('executingAddress is not configured. KeeperHub signs from a Turnkey ' +
                ...
                'app.keeperhub.com, then set KEEPERHUB_EXECUTING_ADDRESS.', 'CONFIG_MISSING_EXECUTING_ADDRESS');
        }
    ...
    async simulate(request) {
        const chainId = SUPPORTED_CHAINS[request.chain ?? this.chain];
        try {
            const response = await this.call(`/api/execute/${request.protocol}/${request.action}`, { chainId, simulateOnly: true, ...request.args });
    ...
    async execute(request) {
        ...
        if (this.simulateFirst) {
            ...
            if (!simulation.ok) {
                throw new KeeperHubError(`Simulation failed, write aborted: ${simulation.reason ?? 'no reason given'}`, 'SIMULATION_FAILED');
            }
        }
        this.consumeBudget();
        const chainId = SUPPORTED_CHAINS[request.chain ?? this.chain];
        const response = await this.call(`/api/execute/${request.protocol}/${request.action}`, {
            chainId,
            ...request.args,
        });
        ...
        if (request.verify) {
            ...
            if (!verified.passed) {
                throw new VerificationError(`KeeperHub reported ${result.status} for ${response.executionId}, but the ` +
                    `read-back did not confirm it` +
                    (request.verify.describe ? `: ${request.verify.describe}` : '') +
                    `. Observed: ${JSON.stringify(verified.observed)}`, verified.observed, response.executionId);
            }
        }
        if (request.idempotencyKey) {
    ...
    async runVerification(spec, execution) {
        try {
    ...
    async status(executionId) {
        const response = await this.call(`/api/execute/status/${encodeURIComponent(executionId)}`, {});
        const receipt = response.receipts?.[0];
        return {
            executionId: response.executionId,
            status: normaliseStatus(response.status),
