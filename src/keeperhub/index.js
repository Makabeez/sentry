 ...
 */
export function keeperhubFromEnv(env = process.env) {
    const apiKey = env.KEEPERHUB_API_KEY;
    if (!apiKey) {
        throw new Error('KEEPERHUB_API_KEY is not set. Get one at https://app.keeperhub.com and add ' +
            'it to .env — the extension will not start without it.');
    }
    const chain = env.KEEPERHUB_CHAIN;
    if (chain && !['ethereum', 'base', 'sepolia'].includes(chain)) {
        throw new Error(`KEEPERHUB_CHAIN is "${chain}", expected one of ethereum, base, sepolia.`);
    }
    ...
    if (budget !== undefined && (!Number.isFinite(budget) || budget < 1)) {
        throw new Error(`KEEPERHUB_WRITE_BUDGET is "${env.KEEPERHUB_WRITE_BUDGET}", expected a positive integer.`);
    }
    return {
        apiKey,
 ...
 */
export function keeperhub(options) {
    const advertise = options.advertise ?? true;
    let executingAddress;
    return {
        name: 'keeperhub',
        build: async (_ctx) => {
        ...
        initialize: async (runtime) => {
            ...
            try {
                ...
                console.log(`[keeperhub] executing from ${wallet.executingAddress} — ` +
                ...
                console.warn(`[keeperhub] executing address unresolved: ` +
